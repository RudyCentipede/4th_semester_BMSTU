...
return_codes_t figure_move(figure_t &figure, const move_data_t &coeff)
{
    ...
    return_codes_t rc = points_move_all(figure.points, coeff);
...
static return_codes_t figure_points_validate(const points_t &points)
{
...
static return_codes_t figure_edges_validate(const edges_t &edges)
{
    return_codes_t rc = SUCCESS;
    if (!edges.data)
        rc = ERROR_EMPTY_DATA;
...
static return_codes_t figure_validate(const figure_t &figure)
{
    return_codes_t rc = SUCCESS;
    if (!figure_points_validate(figure.points) || !figure_edges_validate(figure.edges))
        rc = ERROR_WRONG_FIGURE;
    return rc;
}
