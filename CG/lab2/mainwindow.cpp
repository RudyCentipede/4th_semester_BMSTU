    ...
    , ui(new Ui::MainWindow)
{
    ui->setupUi(this);
    initPoints();
    ui->centerXLineEdit->setText("0");
    ui->centerYLineEdit->setText("0");
}
...
{
    double kx, ky;
    if (!(get_var(kx, ui->kxLineEdit, "Invalid kx")
          && get_var(ky, ui->kyLineEdit, "Invalid ky")))
        return;

    Transform scaling;
    scaling.scale(Point(kx, ky), Point(center_x, center_y));  // Используем сохранённые координаты
...
{
    double alpha;
    if (!get_var(alpha, ui->alphaLineEdit, "Invalid alpha"))
        return;

    Transform rotation;
    rotation.rotate(alpha * M_PI / 180, Point(center_x, center_y));  // Используем сохранённые координаты
...
void MainWindow::on_applyCenterButton_clicked() {
    if (!get_var(center_x, ui->centerXLineEdit, "Invalid Center X") ||
        !get_var(center_y, ui->centerYLineEdit, "Invalid Center Y")) {
        return;
    }
    update();  // Перерисовать
}
...
void MainWindow::paintEvent(QPaintEvent *)
{
    ...
    // Отрисовка центра (не зависит от преобразований фигуры)
    QPointF center = coord(Point(center_x, center_y));
