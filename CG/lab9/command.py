...
def set_pixel(canvas, x, y, colour):
    canvas.create_line(x, y, x + 1, y, fill=colour, width=2)
...
def draw_figure(canvas, figure, figure_colour):
    for i in range(len(figure) - 1):
        canvas.create_line(figure[i], figure[i + 1], fill=figure_colour, width=2)
...
def close_figure(canvas, figure, figure_colour, fig_name):
    ...
    canvas.create_line(figure[-2], figure[-1], fill=figure_colour, width=2)
...
def click_btn(event, figure, clipper, canvas, clipper_colour, colour_figure):
    ...
    if len(clipper) >= 2:
        canvas.create_line(clipper[-2], clipper[-1], fill=clipper_colour, width=2)
...
def cut_off(canvas, figure, clipper, result_colour):
    ...
    for i in range(np):
        canvas.create_line(p[i - 1][0], p[i - 1][1], p[i][0], p[i][1], fill=result_colour, width=2)